...
배포용 설치 파일은 아래 경로에 생성됩니다.

```text
dist\SoundMeterSetup.exe
```

사용자는 `SoundMeterSetup.exe`만 실행하면 됩니다.
...
시작 메뉴에 생성된 `Uninstall Sound Meter`를 실행하면 제거할 수 있습니다.
...
현재 설치 파일은 코드 서명 인증서로 서명되어 있지 않습니다. 공개 배포 규모가 커질 경우 코드 서명 인증서를 적용하는 것을 권장합니다.
